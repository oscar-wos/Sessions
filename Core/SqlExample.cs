    ...
    public partial class Core
    {
        ...
        private async Task CheckPlayerCommand(string command, string playerName, ulong playerId) 
        {
            ...
            */
            ...
            {
                Logger.LogError($"[{nameof(Core)}] Database service is not initialized");
                return;
            }
            ...
            try
            {
                ...
                Logger.LogInformation("[{0}] Command({1}) count: {2}", nameof(Core), commandName, commandCount);

                if (result != null)
                {
                    Logger.LogInformation("[{0}] Command({1}) count: {2}", nameof(Core), result.CommandName ?? string.Empty, result.Count);
                }
            }
            catch (Exception ex)
            {
                Logger.LogError(ex, $"[{nameof(Core)}] Error while checking player commands");
                throw;
            }
        }
    }
